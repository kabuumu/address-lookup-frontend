...
import model.JourneyConfigDefaults.English.LOOKUP_PAGE_MANUAL_ADDRESS_LINK_TEXT
...
class ALFESpec extends WordSpec with MustMatchers with ALFEFixtures {
  ...
  "a resolved config" should {
    ...
    "have default phase banner html" in {
      cfg.phaseBannerHtml must be (JourneyConfigDefaults.English.defaultPhaseBannerHtml(cfg.phaseFeedbackLink))
    }
